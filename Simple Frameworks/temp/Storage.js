...
function sortPockets(pocket, max){
    ...
    for(let i = 0; i< obj.length; i++){
        let item
        let stacksize = getStackSize(item.id)
    ...
    for(let i=0; i < pocket.length; i++){
        if(pocket[i].id == item.id && pocket[i].count + item.count < size ){
            ...
            //到堆叠上限就分割
                ...
                pocket[i].count += item.count
                let _items = splitItems(pocket[i], size)
                pocket[i].count = size
...
function checkItemAvailability(item){
    ...
    //还没整理完的话，按堆叠数切割后继续
    let left = splitItems(data, size)
    let res = checkEmptySlots(left)
...
function makeItemTransfer(items, availability){
    ...
    const transferItems = function(items){
        ...
        while (todo == true){
            ...
            if(items.length == 0){
                ...
                let list = items.map( item =>{return item.name})
    ...
    const merge = function(item, source){
        for(let i in source.stacks){
            let { left, index } = source.stacks
            index.forEach((index)=>{
    ...
    const checkLeft = function(pos){
        ...
        if(leftItem.length > 0){
            ...
            if(availableSlots.available == false){
                let namelist = leftItem.map(item=>{ return item.name })
...
function lostItemsAfterRape(){
    let moneyrate = V.money/10000
    let lostmoney = random(1000, 10000)

    //身上超过50块就有可能丢失，钱越多，丢失概率越大
    if(moneyrate >= 0.5 ){
        let rate = random(100) * moneyrate
        if( rate >= 50 ){
            setMoney(-lostmoney)              
        }  
    }
...
function getItems(itemId, num){
    ...
    if( stack.count + num <= stacksize ){
        ...
        let checked = checkItemAvailability(item)
