...
function onUseDrags(enemy) {
    ...
    effects.forEach(effect => {
        ...
        const max = Math.floor(min * 1.2 + 1.5);
    ...
    const methods = iData.useMethods(type, tags);
...
const iDrugs = [
    {
        ...
        onHigh(min = 1) {
            ...
            const html = `${lanSwitch(drugMsg[this.id].onHigh)}<<ggcontrol>>`;
...

    {
        ...
        onUse(enemy) {
            ...
            // 随机设置两个感官
            const list = ['genital', 'bottom', 'breast', 'mouth'];
        ...
        onHigh(min = 1) {
            ...
            // 第一次显示与持续显示有差分
            const flag = iCandy.getFlag(this.id, 'highonce') ?? 0;

            const html = `${lanSwitch(drugMsg[this.id].onHigh[flag])
    ...
    },
    {
        ...
        onHigh(min = 1) {
            ...
            const _mult = 1 - Math.max(iCandy.getStat(this.id, 'taken') * 0.1, 0.2);
